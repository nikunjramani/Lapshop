package net.in.lapshop.seller.notification;
...
public class ActivitySendPushNotification extends AppCompatActivity implements RadioGroup.OnCheckedChangeListener, View.OnClickListener {
    ...
    private void sendMultiplePush() {
        ...
        addMultipleNotificationToDatabase();
        MyVolley.getInstance(this).addToRequestQueue(stringRequest);
    }

    private void addMultipleNotificationToDatabase() {
        final String title = editTextTitle.getText().toString();
        final String message = editTextMessage.getText().toString();
        final String image = editTextImage.getText().toString();
        for(int i=0;i<devices.size();i++) {
            final int a=i;
            StringRequest stringRequest = new StringRequest(Request.Method.POST, EndPoints.URL_ADD_MUTIPLE_PUSH,
                    new Response.Listener<String>() {
                        @Override
                        public void onResponse(String response) {
                        }
                ...
                public Map<String, String> getParams() throws AuthFailureError {
                    ...
                    params.put("email", devices.get(a));
                    return params;
                }
            };
    ...
    private void addNotificationToDatabase() {
        ...
        final String image = editTextImage.getText().toString();
        final String email = spinner.getSelectedItem().toString();
        StringRequest stringRequest = new StringRequest(Request.Method.POST, EndPoints.URL_ADD_SINGLE_PUSH,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                    }
